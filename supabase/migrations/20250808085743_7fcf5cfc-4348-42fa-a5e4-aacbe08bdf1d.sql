-- Blog Categories table + RLS + triggers, and link to blog_posts (fixed policies without IF NOT EXISTS)
...
-- 2) RLS policies
-- Ensure idempotency by dropping existing policies first
...
DROP POLICY IF EXISTS "Admins can manage all categories" ON public.blog_categories;
...
DROP TRIGGER IF EXISTS trg_blog_categories_set_slug ON public.blog_categories;
